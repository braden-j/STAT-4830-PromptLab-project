"""Token-level labeling for slop detection from (human_text, slop_text) pairs. Used by tests and optional pipelines."""
...
def detect_sloppy_spans(
    ...
    if use_repetition and len(words) >= 2:
        ...
        while i < len(words):
            ...
            if j > i + 1:
                ...
                end_char = min(len(" ".join(words[:j])), len(text))
    ...
    if use_distinct2 and len(words) >= distinct2_window:
        ...
        for start in range(max(0, len(bigrams) - distinct2_window + 2)):
            ...
            if distinct / max(max_bigrams, 1) < distinct2_threshold:
                start_char = len(" ".join(words[: start + 1]))
                end_word_idx = min(start + distinct2_window, len(words))
                end_char = min(len(" ".join(words[:end_word_idx])), len(text))
                spans.add((start_char, end_char))
...
def _chunk_sequence(
    ...
    while pos < n:
        ...
        if len(chunk_ids) < max_length:
            ...
            attn = attn + [0] * pad_len
        chunks.append({"input_ids": chunk_ids, "attention_mask": attn, "token_labels": chunk_labels})
        if pos + max_length >= n:
            break
        pos += stride
    return chunks
    ...
    for human_text, slop_text in pairs:
        ...
        if label_mode == "document":
            for text, label_val in [(human_text, 0), (slop_text, 1)]:
                ...
                token_labels = [label_pad_token_id if special[i] else label_val for i in range(len(ids))]
                results.extend(_chunk_sequence(ids, token_labels, tokenizer, max_length, stride, label_pad_token_id))
            continue
        if label_mode == "span_heuristic":
            if human_text.strip():
                ids_h, offsets_h, special_h = _tokenize_with_offsets(tokenizer, human_text)
                token_labels_h = [label_pad_token_id if special_h[i] else 0 for i in range(len(ids_h))]
                results.extend(_chunk_sequence(ids_h, token_labels_h, tokenizer, max_length, stride, label_pad_token_id))
            ...
            results.extend(_chunk_sequence(ids_s, token_labels_s, tokenizer, max_length, stride, label_pad_token_id))
    return results
