"""
scripts/c4a_reinforce.py — Experiment C4-a: 500-step REINFORCE training run (kl_penalty=0.0)
  ...
  Checkpoints every 100 steps → outputs/c4a_checkpoints/step_NNN/
  Training log → outputs/c4a_log.jsonl
  Eval log     → outputs/c4a_eval.jsonl

Run:
    python scripts/c4a_reinforce.py
    python scripts/c4a_reinforce.py --resume
"""
...
# Output paths
CHECKPOINT_DIR = os.path.join(_REPO_ROOT, "outputs", "c4a_checkpoints")
LOG_PATH       = os.path.join(_REPO_ROOT, "outputs", "c4a_log.jsonl")
EVAL_LOG_PATH  = os.path.join(_REPO_ROOT, "outputs", "c4a_eval.jsonl")
...
def main() -> None:
    ...
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Resume from the latest checkpoint in outputs/c4a_checkpoints/",
    )
    parser.add_argument(
        "--n-rollouts",
        type=int,
    ...
    print("=" * 64, flush=True)
    print("  C4-a REINFORCE (kl_penalty=0.0) — 500-step Training Run", flush=True)
    ...
    print("\n" + "=" * 64, flush=True)
    print("  C4-a REINFORCE TRAINING COMPLETE", flush=True)
