"""
scripts/c4c_reinforce.py — Experiment C4-c: 500-step REINFORCE training run (kl_penalty=0.5)
  ...
  Checkpoints every 100 steps → outputs/c4c_checkpoints/step_NNN/
  Training log → outputs/c4c_log.jsonl
  Eval log     → outputs/c4c_eval.jsonl

Run:
    python scripts/c4c_reinforce.py
    python scripts/c4c_reinforce.py --resume
"""
...
# Output paths
CHECKPOINT_DIR = os.path.join(_REPO_ROOT, "outputs", "c4c_checkpoints")
LOG_PATH       = os.path.join(_REPO_ROOT, "outputs", "c4c_log.jsonl")
EVAL_LOG_PATH  = os.path.join(_REPO_ROOT, "outputs", "c4c_eval.jsonl")
...
# ── Main ──────────────────────────────────────────────────────────────────────
def main() -> None:
    parser = argparse.ArgumentParser(description="C4-c: 500-step REINFORCE training run (kl_penalty=0.5)")
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Resume from the latest checkpoint in outputs/c4c_checkpoints/",
    )
    parser.add_argument(
        "--n-rollouts",
        type=int,
    ...
    print("=" * 64, flush=True)
    print("  C4-c REINFORCE (kl_penalty=0.5) — 500-step Training Run", flush=True)
    ...
    print("\n" + "=" * 64, flush=True)
    print("  C4-c REINFORCE TRAINING COMPLETE", flush=True)
