"""
scripts/c3_grpo.py — Experiment C3: 500-step GRPO training run
  ...
  Algorithm   : GRPO (Group Relative Policy Optimization)
                6 prompt groups × K=8 rollouts/group = 48 total rollouts/step
  ...
  Checkpoints every 100 steps → outputs/c3_checkpoints/step_NNN/
  Training log → outputs/c3_log.jsonl
  Eval log     → outputs/c3_eval.jsonl

Run:
    python scripts/c3_grpo.py
    python scripts/c3_grpo.py --resume
"""
...
N_STEPS           = 500
N_GROUPS          = 6    # number of prompt groups per step
K_ROLLOUTS        = 8    # rollouts per group
N_ROLLOUTS        = N_GROUPS * K_ROLLOUTS  # 48 total, same as C2
MAX_NEW_TOKENS    = 512
...
# Output paths
CHECKPOINT_DIR = os.path.join(_REPO_ROOT, "outputs", "c3_checkpoints")
LOG_PATH       = os.path.join(_REPO_ROOT, "outputs", "c3_log.jsonl")
EVAL_LOG_PATH  = os.path.join(_REPO_ROOT, "outputs", "c3_eval.jsonl")
...
def reinforce_step(
    step_idx: int,
    essays: list[str],          # list of N_GROUPS (6) essays
    policy_tok,
    policy_mdl,
    el_tok, el_mdl,
    fl_tok, fl_mdl,
    ...
    """
    Execute one GRPO step over N_GROUPS essay prompts with K_ROLLOUTS each.
    For each group of K rollouts sharing the same input essay, advantages are
    normalized within the group: adv = (r - group_mean) / (group_std + 1e-8).
    Returns (mean_reward, grad_norm, n_rejected).
    """
    # ── Phase 1: generate K rollouts per essay (no gradient) ─────────────
    policy_mdl.eval()

    # accepted_inputs: list of (prompt_ids, gen_ids, prompt_len, essay_token_len)
    # group_indices[g] holds the indices into accepted_inputs for group g
    accepted_inputs: list[tuple[torch.Tensor, torch.Tensor, int, int]] = []
    el_scores: list[float] = []
    fl_raw:    list[float] = []
    kl_scores: list[float] = []
    group_indices: list[list[int]] = []  # one list per essay group
    n_rejected = 0

    with torch.no_grad():
        for essay in essays:
            essay_token_len = len(
                policy_tok(essay, add_special_tokens=False)["input_ids"]
            )
            prompt_text = build_prompt(essay, policy_tok)

            # Repeat the same prompt K_ROLLOUTS times in a batch
            policy_tok.padding_side = "left"
            enc = policy_tok(
                [prompt_text] * K_ROLLOUTS,
                return_tensors="pt",
                padding=True,
                truncation=True,
                max_length=512,
            ...
            for k in range(K_ROLLOUTS):
                ...
                idx = len(accepted_inputs)
                el_scores.append(el)
                fl_raw.append(fl)
                kl_scores.append(kl_val)
                accepted_inputs.append(
                    (enc["input_ids"][k].unsqueeze(0).cpu(), gen_ids.cpu(), prompt_len, essay_token_len)
                )
    ...
    # GRPO advantage normalization: within each group of K rollouts sharing the
    # same input essay, normalize rewards to zero mean and unit variance.
    advantages = [0.0] * n_accepted
    for group_idx in group_indices:
        if not group_idx:
            continue
        group_rewards = [rewards[i] for i in group_idx]
        g_mean = sum(group_rewards) / len(group_rewards)
        g_var  = sum((r - g_mean) ** 2 for r in group_rewards) / len(group_rewards)
        g_std  = g_var ** 0.5
        for i, r in zip(group_idx, group_rewards):
            advantages[i] = (r - g_mean) / (g_std + 1e-8)
...
# ── Main ──────────────────────────────────────────────────────────────────────
def main() -> None:
    parser = argparse.ArgumentParser(description="C3: 500-step GRPO training run")
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Resume from the latest checkpoint in outputs/c3_checkpoints/",
    )
    parser.add_argument(
        "--max-new-tokens",
        type=int,
    ...
    print("=" * 64, flush=True)
    print("  C3 GRPO (K=8 groups×6) — 500-step Training Run", flush=True)
    print("  STAT 4830 PromptLab Deslopifier", flush=True)
    print("=" * 64 + "\n", flush=True)

    device = get_device()

    # Load data — pool of essays; sample N_GROUPS per step
    ...
    # Header
    print(
        f"Config: {N_STEPS} steps × {N_GROUPS} groups × {K_ROLLOUTS} rollouts  |  "
    ...
    for step in range(start_step, N_STEPS):
        mean_r, gnorm, n_rej = reinforce_step(
            step_idx=step,
            essays=random.sample(essays_pool, N_GROUPS),
    ...
    print("\n" + "=" * 64, flush=True)
    print("  C3 GRPO TRAINING COMPLETE", flush=True)
